...
import "./Chat.css"
import "../commandPages/Commands.css"
...
const Chat:React.FC<IChatProps>=({Author})=> {
	const [ state, setState ] = useState<{message:string,author:string}>({ message: "", author: Author })
					...
					<div className="card">
						<form id="messengerStyle" onSubmit={onMessageSubmit}>
							...
							<div className="name-field">
								<TextField name="author" onChange={(e: React.ChangeEvent<HTMLInputElement>) => onTextChange(e)} value={state.author} label="Name"/>
							</div>
							<div>
								<TextField
									name="message"
